...
• 随机生成一个CT，然后对该CT 进行局部搜索。 这种模因方法涉及在并置阶段结束时的每个模板更新之前对竞争模板进行局部搜索。
   • 使用包含特定结构的完全指定的总体成员作为 CT。 每个种子 CT 都使用已知的 alpha 螺旋和 beta 折叠二面角硬编码到 fmGA 中。 对于具有这些二级结构之一的蛋白质，该算法有望以更快的速度获得更好的适应度值。
   • 使用全息CT。 该过程生成奇数个 CT 并将它们合并到一个模板中，在此方法中称为 panmetic CT。 预计合并将采用每个 CT 的最佳组件并将它们组合在一起。  CT 可以全部是随机的，也可以是随机与包含结构的组合。
   • 使用通过上述方法开发的多个 CT。 这种方法允许进行更多探索，因为每个种群成员都使用多个模板进行评估，因此有可能通过搜索景观的不同区域来找到更好的解决方案。  GA 的强大功能的一个主要来源来自所谓的隐式并行性，即将搜索工作同时分配到搜索空间的许多区域。 隐式并行的完美实现意味着并行采样大量不同的短、低阶高适应度模式，从而为交叉算子提供足够多样的基本构建块，以将它们组合成更高度拟合、更复杂的构建块。 然而，传统的遗传算法存在过早收敛的问题，在获得更多进步之前，在次优区域的某些图式上发生了相当大的固定。
   在过早收敛的例子中，搭便车已被确定为主要障碍，它通过降低各种有益构建块的采样频率来限制隐式并行性。
   简而言之，在某些模式上搭便车的不相关等位基因可能会传播到下一代并淹没其他可能有利的构建块，从而阻止对构建块的独立采样。 因此，交叉在组合构建块中的功效受到所需种群多样性损失的限制。 因此，研究人员考虑了一种所谓的理想化遗传算法 (IGA)，它允许每个人完全独立地进化； 因此，新样本被独立地提供给每个模式区域，并抑制搭便车。 然后在假设 IGA 事先了解所需模式的情况下，他们推导出了 IGA 为找到皇家路函数的最佳值所需的函数评估数量的下限。
