...
研究人员已经提出了遗传算法的各种变体。遗传算法的变体大致分为五大类，即实数和二进制编码、多目标、并行、混沌和混合遗传算法。这些算法及其应用的优缺点已在前面的小节中讨论过
...
二进制表示用于编码遗传算法，称为二进制遗传算法。遗传算子也被修改以执行搜索过程。Payne和Glen[153]开发了一种二元遗传算法来识别分子间的相似性。他们用二进制表示分子的位置和构象。然而，该方法具有较高的计算复杂度。龙岩等人[203]研究了使用二进制遗传算法（BGA）设计风电场的三种不同方法。他们的方法产生了更好的适应值和农场效率。Shukla等人[185]利用BGA进行特征子集选择。他们使用互信息最大化的概念来选择重要特征。BGA面临着汉明悬崖、不均匀的模式和难以达到精度的问题[116，199]。

##### 4.1.2 Real-coded GAs

实际编码的GA（RGAS）已广泛用于各种现实生活应用。 染色体的表示与现实生活问题密切相关。  RGA的主要优点是强大，高效，准确。 然而，RGAS通常有过早收敛这一问题。 研究人员正在研究RGA来提高他们的表现。  RGA大部分是通过修改交叉，突变和选择运算符开发的
...
**Mutation operators** 变异算子在种群中产生多样性。 在应用突变过程中必须解决两个主要挑战。 首先，变异算子应用于种群的概率是多少？ 其次，变异过程后染色体中产生的异常值应该如何处理？  Michalewicz [135] 提出了 RGA 的统一和非统一变异算子。  Michalewicz 和 Schoenauer [136] 开发了一种统一突变的特例。 他们开发了边界突变。Deep 和 Thakur [38] 提出了一种基于幂律的新型变异算子，命名为幂变异。  Das 和 Pratihar [30] 提出了基于方向的指数变异算子。 他们使用变量的方向信息。  Tang 和 Tseng [196] 提出了一种新的变异算子，用于提高 RCGA 的性能。他们的方法快速而可靠。 但是，它在某些应用程序中停留在局部最优。 德布等人。  [35] 开发了用于 RCGA 的多项式突变。它提供了更好的探索。 然而该算法的收敛速度很慢，并且停留在局部最优。 卢卡修斯等人。  [129] 提出了一种实数编码遗传算法（RCGA）。它简单易行。 然而，它存在局部最优问题。王等人。  [205] 开发了多后代遗传算法并研究了它们在单点交叉上的表现。 王等人。  [206] 阐述了多后代遗传算法的理论基础。 该方法的性能优于非多后代遗传算法。帕塔奈克等人。  [152] 提出了 RCGA 的改进。

![image-20210803134921294](C:\Users\Lenovo\AppData\Roaming\Typora\typora-user-images\image-20210803134921294.png)

#### 4.2 Multiobjective GAs

MultiobjectiveGA(MOGA)是简单GA的修改版本。MOGA在适应度函数分配方面与GA不同。 其余步骤与 GA 类似。 多目标遗传算法的主要动机是在目标空间中生成最优的帕累托最优，这样就不会在不干扰其他适应度函数的情况下进一步增强任何适应度函数[123]。收敛性、多样性和覆盖率是多目标遗传算法的主要目标。 多目标 GA 大致分为两类，即基于帕累托的多目标 GA 和基于分解的多目标 GA [52]。 这些技术在前面的小节中讨论过。
...
与其他方法相比，主从并行 GA (MS-PGA) 中使用了大量处理器。 可以通过增加处理器的数量来增加适应度函数的计算。 洪等人。  [79] 使用 MS-PGA 来解决数据挖掘问题。 模糊规则与并行遗传算法一起使用。 适应度函数的评估是在从机上进行的。 然而，它受到高计算时间的影响。  Sahinzo [174] 为无人机寻路问题实现了 MS-PGA。 遗传算子在处理器上执行。 他们使用了四核的多核 CPU。 选择和适应度评估是在从机上完成的。  MS-PGA 应用于[127]中的流量分配问题。 他们在新加坡国立大学使用了 30 个处理器来解决这个问题。杨等人。  [213] 开发了一种基于网络的并行遗传算法。 他们在分布式环境中实现了 NSGA-II 的主从版本。 然而，该系统本质上是复杂的。
...
在过去的几十年里，研究人员正在研究细粒度并行遗传算法 (FGPGA) 的迁移策略。波尔塔等人。  [161] 利用时钟时间作为迁移频率，这与世代无关。 他们使用了非均匀结构和静态配置。 选择最佳解决方案进行迁移，最差解决方案替换为迁移解决方案。  Kurdi [115] 使用了自适应迁移频率。 迁移过程开始，直到连续十代后获得的解决方案没有变化。 使用了非均匀和动态结构。 在[209]中，局部最优解被同步并形成全局最优解。 全局最佳解决方案被转移到所有处理器以供父执行。 迁移频率取决于世代数。 他们使用统一的结构和固定的配置。 张等人[220] 使用并行遗传算法解决无线网络的集合覆盖问题。 他们使用分而治之的策略将种群分解为子种群。 此后，将遗传算子应用于局部解，并使用 Kuhn-Munkres 合并局部解。

##### 4.3.3 Coarse grained parallel GA

皮内尔等人。  [158] 提出了一个 GraphCell。 群体用随机值初始化，一个解用最小-最小启发式技术初始化。  448 个处理器被用来实现所提出的方法。 然而，由于性质复杂，粗粒度并行 GA 较少使用。 混合并行 GA 广泛用于各种应用。 沙耶吉等人。  [182] 提出了一个基于池的伯明翰集群 GA。 主节点负责管理全球人口。 从节点从全局人口中选择解决方案并执行它。  240 个处理器用于计算。 罗伯格等人。  [170] 使用混合方法来优化逆变器的开关角。 他们使用了四种不同的适应度函数计算策略。 如今，GPU、云和网格是并行 GA 最流行的硬件 [198]。

#### 4.4 Chaotic GAs

GA 的主要缺点是过早收敛。 混沌系统被合并到 GA 中以缓解这个问题。 混沌遗传算法的多样性消除了早熟收敛。 交叉和变异算子可以用混沌图代替。  Tiong 等人[197] 将混沌图集成到 GA 中以进一步提高准确性。 他们使用了六种不同的混沌图。  Logistic、Henon 和 Ikeda 混沌遗传算法的性能优于经典遗传算法。 然而，这些技术的计算复杂度很高。

Ebrahimzadeh 和 Jampour [48] 使用 Lorenz 混沌作为遗传算法的遗传算子来消除局部最优问题。 然而，所提出的方法无法找到熵和混沌图之间的关系。  Javidi 和 Hosseinpourfard [87] 利用两个混沌图，即逻辑图和帐篷图来生成混沌值，而不是随机选择初始种群。 提出的混沌遗传算法比遗传算法表现更好。 然而，这种方法的计算复杂度很高。 富尔特斯等人。  [60]将熵整合到混沌遗传算法中。 通过混沌映射修改控制参数。 他们研究了熵和性能优化之间的关系。

混沌系统也用于多目标和混合 GA。  Abo-Elnaga 和 Nasr [5] 将混沌系统集成到改进的 GA 中以解决双层规划问题。 混沌有助于所提出的算法缓解局部最优并增强收敛性。 塔希尔等人。[193] 提出了一种用于医疗保健中特征选择的二元混沌遗传算法。 他们使用混沌图来初始化种群，并将修改后的复制算子应用于种群。 徐等人。[115] 提出了一种用于频谱分配的混沌混合免疫遗传算法。 所提出的方法利用了混沌算子和免疫算子的优点。 然而，这种方法存在参数初始化问题。

#### 4.5 Hybrid GAs

遗传算法可以很容易地与其他优化方法混合以提高其性能，例如图像去噪方法、化学反应优化等。混合遗传算法与其他方法的主要优点是更好的解决方案质量、更高的效率、可行解决方案的保证以及优化的控制参数 [51]。 从文献中可以看出，遗传算法的抽样能力受种群规模的影响很大。 为了解决这个问题，局部搜索算法如模因算法、鲍德温、拉马克和局部搜索等已经与 GAs 集成。 这种整合在集约化和多样化之间提供了适当的平衡。  GA 中的另一个问题是参数设置。 寻找合适的控制参数是一项繁琐的工作。 其他元启发式技术可以与 GA 一起使用来解决这个问题。 混合 GA 已被用于解决前面小节中提到的问题 [29, 137, 186]。

##### 4.5.1 Enhance search capability

GAs 已与局部搜索算法集成以减少遗传漂移。 在局部搜索中引入了显式细化运算符以产生更好的解决方案。  El-Mihoub 等人。[54] 建立了局部搜索概率对遗传算法种群规模的影响。 埃斯皮诺萨等人。[50] 研究了局部搜索对减少 GA 种群规模的影响。 不同的搜索算法已与 GA 集成以解决实际应用

##### 4.5.2 Generate feasible solutions

在复杂的高维问题中，遗传算法的遗传算子会产生不可行的解。  PMX 交叉为基于订单的问题生成不可行的解决方案。 开发距离保持交叉算子是为了生成旅行商问题的可行解决方案 [58]。 使用基因池运算符而不是交叉来生成数据聚类的可行解[19]。  Konak 和 Smith [108] 将切饱和算法与 GA 集成在一起，用于设计通信网络。 他们使用均匀交叉来产生可行的解决方案

##### 4.5.3 Replacement of genetic operators

有可能用其他搜索技术替换第 3.2 节中提到的遗传算子。  Leng [122] 开发了一种引导 GA，它利用引导本地搜索的惩罚。 这些惩罚用于适应度函数以提高 GA 的性能。  Headar 和 Fukushima [74] 使用单工交叉代替标准交叉。 在[195]中用模拟退火代替了标准变异算子。 量子计算的基本概念用于提高 GA 的性能。 启发式交叉和爬山算子可以集成到遗传算法中解决三匹配问题

##### 4.5.4 Optimize control parametersators

遗传算法的控制参数在保持集约化和多样化之间的平衡方面起着至关重要的作用。 模糊逻辑能够估计 GA 的适当控制参数 [167]。 除此之外，遗传算法还可用于优化其他技术的控制参数。  GA 已被用于优化中性网络的学习率、权重和拓扑结构 [21]。 遗传算法可用于估计控制器中模糊隶属度的最优值。 它还用于优化 ACO、PSO 和其他元启发式技术的控制参数 [156]。 表 9 中提到了知名 GA 的比较分析。

### 5 Applications

遗传算法已经以高准确率应用于各种 NP-hard 问题。有几个应用领域已经成功地应用了 GA。

#### 5.1 Operation management

GA 是一种有效的元启发式方法，可用于解决运营管理 (OM) 问题，例如设施布局问题 (FLP)、供应网络设计、调度、预测和库存控制

##### 5.1.1 Facility layout

达塔等人。  [32]利用遗传算法解决单行设施布局问题（SRFLP）。 对于 SRFLP，GA 的修改交叉和变异算子产生了有效的解决方案。 他们将 GA 应用于由 60-80 个实例组成的大型问题。 然而，它存在参数依赖问题。  Sadrzadeh [173] 提出了用于多线 FLP 的 GA 有多种产品。 这些设施使用变异和启发式算子进行聚类。 与其他算法相比，从所提出的 GA 中获得的总成本降低了 7.2%。吴等人。  [211] 实施分层 GA 以找出细胞制造系统的布局。 然而，遗传算子对遗传算法的性能影响很大。 艾洛等人。  [7] 为 FLP 提出了 MOGA。 他们在 20 个不同部门的布局中使用了 MOGA。  Palomo-Romero 等人。  [148] 提出了一个岛模型 GA 来解决 FLP。 所提出的技术保持了种群多样性并产生了比现有技术更好的解决方案。 然而，该技术受到可用于改善种群的不正确迁移策略的影响。  GA 及其变体已成功应用于 FLP [103, 119, 133, 201]。

##### 5.1.2 Scheduling

GA 在解决诸如作业车间调度 (JSS)、集成过程计划和调度 (IPPS) 等调度问题方面表现出卓越的性能 [119]。 为了提高上述调度领域的性能，研究人员开发了各种遗传表示 [12, 159, 215]、遗传算子，并将遗传算法与其他方法混合 [2, 67, 147, 219]。

![image-20210803140449573](C:\Users\Lenovo\AppData\Roaming\Typora\typora-user-images\image-20210803140449573.png)

##### 5.1.3 Inventory contro

除了调度，库存控制在 OM 中也扮演着重要的角色。 缺货和销售损失是库存控制的两种主要方法 [119]。 希萨特等人。  [76]利用位置库存模型来找出仓库的数量和位置。 在 GA 及其变体的目标函数中添加了各种设计约束以解决库存控制问题

##### 5.1.4 Forecasting and network design

预测是 OM 的重要组成部分。 研究人员正致力于预测金融交易、物流需求和游客人数。  GA 已与支持向量回归、模糊集和神经网络 (NN) 相结合，以提高其预测能力 [22, 78, 89, 178, 214]。 供应网络设计极大地影响运营计划和调度。 大多数研究文章都集中在设施的容量限制上 [45, 184]。 多产品多期问题增加了供应网络的复杂性。 为了解决上述问题，GA 已与其他技术相结合 [6, 45, 55, 188, 189]。 多目标 GA 也用于优化成本、利润、碳排放等 [184, 189]。

#### 5.2 Multimedia

GAs 已经应用于多媒体的各个领域。 一些著名的多媒体领域是加密、图像处理、视频处理、医学成像和游戏。

##### 5.2.1 Information security

由于多媒体应用的发展，图像、视频和音频通过互联网从一个地方传输到另一个地方。 文献中已经发现，图像在传输过程中更容易出错。 因此，需要采用加密、水印、密码等图像保护技术。 经典的图像加密技术需要输入参数进行加密。 输入参数的错误选择将产生不适当的加密结果。  GA 及其变体已被用于选择适当的控制参数。  Kaur 和 Kumar [96] 开发了一种多目标遗传算法来优化混沌图的控制参数。 密钥是使用 beta 混沌映射生成的。 生成的密钥用于加密图像。 并行 GA 也用于加密图像 [97]。

##### 5.2.2 Image processing

主要的图像处理任务是预处理、分割、目标检测、去噪和识别。 图像分割是解决图像处理问题的重要步骤。 分解/分割图像需要大量的计算时间。 为了解决这个问题，使用 GA 是因为它们具有更好的搜索能力 [26, 102]。 增强是一种提高图像质量和对比度的技术。 分析给定图像需要更好的图像质量。  GA 已被用于增强自然对比度和放大图像 [40, 64, 99]。 一些研究人员正在研究粗糙集与遗传算法的混合，以合并噪声和颜色属性。  GA 已用于从给定图像中去除噪声。  GA 可以与模糊逻辑混合以对噪声图像进行去噪。 基于遗传算法的恢复技术可用于去除给定图像中的雾霾、雾气和烟雾 [8, 110, 146, 200]。 目标检测和识别是现实世界中的一个具有挑战性的问题。 高斯混合模型在检测和识别过程中提供更好的性能。 控制参数通过 GA [93] 进行优化。

##### 5.2.3 Video processing

视频分割已广泛用于模式识别和计算机视觉。 有一些与视频分割相关的关键问题。 这些是将物体与背景区分开来并确定准确的边界。  GA 可用于解决这些问题 [9, 105]。  Chao el al 已经成功地实现了用于手势识别的 GA。  [81] 使用遗传算法进行手势识别。 他们应用了遗传算法，发现机器人视觉的准确率为 95%。  Kaluri 和 Reddy [91] 提出了一种基于自适应遗传算法的方法以及用于手势识别的模糊分类器。 他们报告说，与提供 79% 准确率的现有方法相比，识别率提高了 85%。除了手势识别，人脸识别在刑事识别、无人驾驶、监控、机器人等领域也发挥着重要作用。  GA 能够处理遮挡、方向、表情、姿势和光照条件 [69, 95, 109]。

##### 5.2.4 Medical imaging

遗传算法已应用于医学成像，例如 MRI 中的边缘检测和 CT 扫描图像中的肺结节检测 [100, 179]。 在 [120] 中，作者使用模板匹配技术和 GA 来检测 CT 图像中的结节。  Kavitha 和 Chelamuthu [179] 使用基于 GA 的区域生长方法来检测脑肿瘤。  GAs 已应用于从病理对象中捕获的医学预测问题。  Sari 和 Tuna [176] 使用 GA 来解决生物力学中出现的问题。 它用于在检查期间预测病理。  Ghosh 和 Bhattachrya [62] 使用元胞自动机实现了序列遗传算法，用于对冠状病毒疾病 19 (COVID-19) 数据进行建模。  GA 可以以并行模式应用以在生物数据集中查找规则 [31]。 作者提出了一种并行 GA，它通过将过程划分为小的子代并并行评估每个单独解决方案的适应度来运行。 遗传算法用于医学和其他相关领域。Koh 等人 [61] 提出了一种基于遗传算法的方法，用于评估给定药物的不良反应。

##### 5.2.5 Precision agriculture

遗传算法已应用于与精准农业相关的各种问题。 主要问题是作物产量、杂草检测和农业设备的改进。  Pachepsky 和 Acock [145] 实施 GA 以使用遥感图像分析土壤中的含水量。 作物产量可以通过土壤中存在的水的容量来预测。 杂草识别是通过 [142] 中的 GA 完成的。 他们使用航拍图像对植物进行分类。 在[124]中，彩色图像分割用于区分杂草和植物。Peerlink 等。  [154] 确定了农田不同部分的适当肥料比例。 他们GA用于测定麦田中的氮。 水灌溉系统中的能源要求可以通过将其视为多目标优化问题来优化。 在 SMART 农场中，所需的灌溉量和电力需求不断变化。 因此，遗传算法可以应用于灌溉系统以降低功率需求[33]。

##### 5.2.6 Gaming

GAs 已成功用于 gomoku 等游戏。 在 [202] 中，作者表明基于 GA 的方法可以找到比基于普通树的方法具有最高适应度的解决方案。 然而，在基于实时策略的游戏中，基于 GA 的解决方案变得不太实用 [82]。 考虑到环境约束以及避免到达给定目的地的障碍，GAs 已被用于路径规划问题。  Burchardt 和 Salomon [18] 描述了足球比赛路径规划的实现。  GA 可以通过二维运动场的坐标点对路径规划问题进行编码，从而产生可变长度的解决方案。 路径规划中的适应度函数考虑了路径的长度以及足球运动员的碰撞避免项。

#### 5.3 Wireless networking

由于 GA 的适应性、可扩展性和易于实现，它已被用于解决无线网络的各种问题。 无线网络的主要问题是路由、服务质量、负载平衡、本地化、带宽分配和信道分配 [128, 134]。  GA 已与其他元启发式混合用于解决路由问题。混合 GA 不仅在节点对之间产生有效的路由，而且还用于负载平衡 [24, 212]。

##### 5.3.1 Load balancing

如今，多媒体应用需要延迟和带宽的服务质量 (QoS) 需求。 各种研究人员正在研究基于 QoS 的解决方案的 GA。GA 为复杂网络生成最佳解决方案 [49]。 罗伊等人。  [172] 针对多播 QoS 路由问题提出了多目标遗传算法。  GA 与 ACO 和其他搜索算法一起使用，以找到具有所需 QoS 指标的最佳路由。 负载平衡是无线网络中的另一个问题。  Scully 和 Brown [177] 使用 MicroGA 和 MacroGA 在网络的各个组件之间分配负载。 他等人。  [73] 实施 GA 来确定无线传感器网络中的平衡负载。 程等人。  [25] 利用分布式 GA 和多种群方案进行负载平衡。 他们使用负载平衡指标作为 GA 中的适应度函数。

##### 5.3.2 Localization

确定无线节点位置的过程称为定位。 它在灾害管理和军事服务中发挥着重要作用。 云等人。  [216] 使用具有模糊逻辑的 GA 来找出根据信号强度分配的权重。 张等人。  [218] 将 GA 与模拟退火 (SA) 混合，以确定无线节点的位置。  SA用作局部搜索以消除早熟收敛。

##### 5.3.3 Bandwidth and channel allocation

适当的带宽分配是一项复杂的任务。 已经开发了 GA 及其变体来解决带宽分配问题 [92, 94, 107]。  GA 用于研究具有 QoS 约束的带宽分配。  GA 的适应度函数可能包括资源利用率、带宽分布和计算时间 [168]。 信道分配是无线网络中的一个重要问题。 信道分配的主要目标是同时优化信道数量和分配频率的重用。 朋友等。  [59] 使用分布式孤岛 GA 来解决认知无线电网络中的信道分配问题。 振华等。  [221] 为信道分配实施了一种改进的免疫遗传算法。 他们使用了不同的编码方案和免疫算子Pinagapany 和 Kulkarni [157] 开发了一种并行遗传算法来解决静态和动态信道分配问题。 他们使用十进制编码方案。 表 10 总结了 GA 及其变体的应用。

### 6 Challenges and future possibilities

在本节中，讨论了 GAs 实施过程中面临的主要挑战，然后是可能的研究方向

#### 6.1 Challenges

尽管有几个优点，但为了未来的进步和遗传算法的进一步发展，还需要解决一些挑战。 一些主要挑战如下：

##### 6.1.1 Selection of initial population

初始种群一直被认为是影响遗传算法性能的重要因素。 种群规模也影响解决方案的质量[160]。 研究人员认为，如果考虑大量人口，则该算法需要更多的计算时间。然而，种群规模少可能导致解决方案不佳[155]。 因此，找到合适的种群规模始终是一个具有挑战性的问题。  Harik 和 Lobo [71] 使用自适应方法调查了所需的种群规模大小。 他们使用了两种方法，例如（1）在执行算法之前使用自适应，其中种群大小保持不变；（2）在算法执行期间使用自适应，其中种群大小受适应度影响 功能。

##### 6.1.2 Premature convergence

过早收敛是 GA 的常见问题。 它可能导致等位基因丢失，从而难以识别基因 [15]。 过早收敛表明，如果优化问题出现得太早，结果将是次优的。 为了避免这个问题，一些研究人员建议应该使用多样性。 应该使用选择压力来增加多样性。 选择压力是有利于 GA 初始种群中更好个体的程度。 如果选择压力 (SP1) 大于某个选择压力 (SP2)，则使用 SP1 的种群应该大于使用 SP2 的种群。 较高的选择压力会降低可能导致过早收敛的种群多样性[71]。

收敛性必须正确处理，以便算法找到全局最优解而不是局部最优解（见图 8）。 如果最优解位于不可行解附近，那么遗传算法的全局特性可以与其他算法的局部特性相结合，例如禁忌搜索和局部搜索。 遗传算法的全局性和禁忌搜索的局部性在集约化和多样化之间提供了适当的平衡

##### 6.1.3 Selection of efficient fitness functions

适应度函数是驱动力，在算法的每次迭代中对选择最适合的个体起着重要作用。 如果迭代次数少，则可以调整代价高昂的适应度函数。 迭代次数的增加可能会增加计算成本。 适应度函数的选择取决于计算成本及其适用性。 在 [46] 中，作者使用 Davies-Bouldin 索引对文档进行分类。
