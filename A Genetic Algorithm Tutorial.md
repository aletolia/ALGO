...
如果孤立地检查搜索空间中的每个点，则确定每个字符串都是 $2^L- 1$ 超平面分区的成员并不能提供太多信息。 这就是为什么基于群体的搜索的概念对遗传算法至关重要。 一组样本点提供了有关众多超平面的信息； 此外，低阶超平面应该由总体中的许多点进行采样。  （在本文的后续部分中更详细地重新研究了这个问题）遗传算法的内在或隐式并行性的一个关键部分来自这样一个事实，即在评估一组字符串时对许多超平面进行采样（Holland 1975）； 事实上，可以说采样的超平面比总体中包含的字符串数量多得多。 许多每次评估单个字符串时，都会以隐式并行方式评估不同的超平面（Holland 1975:74）； 但它是评估一组点的累积效应，提供有关任何特定超平面子集的统计信息。1 隐式并行意味着许多超平面竞争同时并行解决。 该理论表明，通过复制和重组过程，竞争超平面的图式根据位于这些超平面分区中的字符串的相对性来增加或减少它们在总体中的表示。

由于遗传算法对字符串群体进行操作，因此可以跟踪代表群体中特定超平面的单个模式的比例表示，并指示当基于适合度的选择与交叉结合时，该超平面是否会随着时间的推移增加或减少其在群体中的表示 从人口中现有的字符串中产生 o 弹簧。
